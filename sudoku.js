...
var SudokuSolver = (function (testable) {
  ...
  function solve(boardString) {
    ...
    return recursiveSolve(boardArray.join(""));
  }
  ...
  function collectionIsValid(collection) {
    ...
    for (var i = 0; i < collection.length; i++) {
      if (collection[i] !== "-" && numCounts[collection[i]] !== undefined) {
        return false;
      } else if (collection[i] !== "-") {
        numCounts[collection[i]] = true;
      }
    }
    return true;
  }
  ...
  if (testable) {
    ...
    // Expose only necessary functions for regular usage
    solver = {
      solve: solve,
      solveAndPrint: solveAndPrint
    };
